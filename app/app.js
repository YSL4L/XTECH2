    ...
    if (elt) {
      ...
      return function () {
        if (!this.hasAttribute("data-processed")) {
          ...
          if (
            content &&
            !childExists(this.firstElementChild, content.nodeName)
          ) {
  ...
  function childExists(elt, name) {
    if (elt && elt.nodeName == name) {
      ...
      return (
        ...
        childExists(elt.nextElementSibling, name)
      );
    }
  }
        ...
        if (elt.hasAttribute(replacements[2])) {
          if (replacements[1] && utilities[replacements[1]]) {
            result = result.replace(
              replacements[0],
              utilities[replacements[1]](elt.getAttribute(replacements[2]))
            );
          } else {
            result = result.replace(
              replacements[0],
  ...
  function define(names) {
    for (let name of names) {
      try {
        ...
        if (this.debug) {
          console.log(
            tagName(name) + " couldn't be registered or is already registered."
          );
          console.log(error);
        }
      }
    ...
    addStyle(doc, data) {
      if (this.hasStyle) {
        doc
          ...
          .item(0)
          .appendChild(
            data.getElementsByTagName("style").item(0).cloneNode(true)
          );
      }
    }
